#' @name parallelnh_OSX
#' @title Parallel NewHybrids - OS X version
#'
#' @description \code{paralllelnh_OSX} allows NewHybrids (Anderson and Thompson 2002) to be run in parallel. It does so by creating a vector of file names to be run, that are held within a single folder. paralleleNH_OSX then assigns a job to each of the cores available in the computer. As each task finishes, parallelnh_OSX assigns a new analysis to the idle core.
